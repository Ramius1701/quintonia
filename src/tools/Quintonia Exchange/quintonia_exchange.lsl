...
// -----------------------------------------
//
float   VERSION = 5.5;    // 21 January 2022
...
doReset()
{
    ...
    if (llGetInventoryType("qex") == INVENTORY_NOTECARD) llRemoveInventory("qex");
    llSleep(1.0);
    ExchangeID = "*";
    osMakeNotecard("qex", ExchangeID);
    if (llGetInventoryType("tally") == INVENTORY_NOTECARD) llRemoveInventory("tally");
    llSleep(1.0);
    status = "";
    llSetObjectDesc("");
    llSleep(2.0);
