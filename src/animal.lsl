    ...
    //  Rezzed products match the language of the animal
    ...
    //  Data now saved on root prim (lagacy values) and first linked prim (newer values)
    //  Added an_config option to disbale saving status nc and just rely on prim descriptions  saveStatusNc = TRUE;     // USE_NC=1
    //  Set notecard saving of animal settings to only happen once every 12 hours 
    //
    // New text
    ...
    float    VERSION = 5.9;    // 5.9 BETA 1  20 February 2022
    ...
    integer  saveStatusNc        = FALSE;               // USE_NC=0            Set to 1 to  save animal status to a notecard about every 12 hours. 0 disables, which can reduce lag/load on server but may result in animals reverting to 0 days old on OAR restores and possibly region restarts 
    ...
    setConfig(string str)
    {
        ...
        if (llList2String(tok,0) != "")
        {
            ...
            else if (cmd == "USE_NC") saveStatusNc = (integer)val;
    ...
    {       
        ...
        if (status != "waitStatusNC")
        {
            ...
            // Since won't all fit in single desc (max 128 chars) put final data on another prim
            string codedDesc2 = (string)labelType+";" +neatVector(initpos)+";" +qsFloat2String(scaleFactor, 1, FALSE) +";" +(string)AN_AUTO_POO +";" +neatRotation(savedRot) +";" +neatVector(savedPos) +";" +(string)SLEEP_MODE +";" +(string)feedPet;
            if (isPet == TRUE) codedDesc1 = "X;"+codedDesc1; else codedDesc1 = "A;"+codedDesc1;
            //
            // Only save to notecard around once every 12 hours
            if ((llGetUnixTime()-lastSavedTs > 43200) && (saveStatusNc == TRUE))
            {  
                ...
                // save to notecard
                osMakeNotecard(statusNC, codedDesc1 +";" +codedDesc2);
            ...
            if (isHuman == FALSE)
            {
                ...
                if (dataPrimNumber != -1) llSetLinkPrimitiveParamsFast(dataPrimNumber, [PRIM_DESC, codedDesc2]);
            }
        }
    }
    ...
    loadState()
    {
        ...
        if (status != "waitStatusNC")
        {
            ...
            if ((llGetSubString(llGetObjectDesc(), 0, 0)  == "A") || (llGetSubString(llGetObjectDesc(), 0, 0)  == "X"))
            {
                desc  = llParseStringKeepNulls(llGetObjectDesc(), [";"], []);
                desc += llParseStringKeepNulls(llList2String(llGetLinkPrimitiveParams(dataPrimNumber, [PRIM_DESC]), 0), [";"], []);
            }           
    ...
    refresh()
    {
        ...
        if (isPet == FALSE) age = (ts-createdTs);
        if (epoch == 0)
        {
    ...
    rezManure(key id)
    {
        llMessageLinked(LINK_SET, 0, "REZ_PRODUCT|" +PASSWORD +"|" +(string)id +"|" +MANURE_OBJECT, NULL_KEY);
    ...
    default
    {
        ...
        object_rez(key id)
        {
            ...
            if (llKey2Name(id) == llGetObjectName()) //Child
            {
                ...
                // Give the animal the language caeds
...
llRemoteLoadScriptPin(id, "animal-heaven", 999, TRUE, 1);
                ...
                llSleep(1);
                messageObj(id, "INIT|"+PASSWORD+"|"+babyParams+"|"+languageCode+"|"+SURFACE+"|"+(string)CHATTY+"|"+(string)labelType+"|"+(string)RADIUS+"|"+(string)AN_AUTO_POO+"|"+AN_FEEDER+"|"+(string)SLEEP_MODE +"|" +(string)saveStatusNc +"|" +(string)isPet);
            }
            else
            {
        ...
        dataserver(key kk, string m)
        {
            ...
            if (llList2String(tk,1) == PASSWORD)
            {
                ...
                {
                    // INIT|PASSWORD|geneA|fatherGene|newName|lang|SURFACE|CHATTY|labelType|RADIUS|AN_POO|FEEDER|SLEEP|saveStatusNc|isPet
                    // If command sent from rezzer geneA and fatherGene are 0
                    if (llList2String(tk, 14) != "") isPet = llList2Integer(tk, 14);
                    if (llList2String(tk, 13) != "") saveStatusNc = llList2Integer(tk, 13);
                    ...
                    if (LAYS_EGG==0) say(0, TXT_HELLO, NULL_KEY); else status = "NEWBORN";
                    refresh();
                    move();
                    llSetTimerEvent(2);
                }
